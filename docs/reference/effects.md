...
| `"host <name> not in http whitelist"` | Whitelist denies this host. Deny-by-default since #136 H2. Set an access class (`"all"`, `"local"`, `"external"`) and/or explicit hosts via `bridge.set_http_whitelist`. The class is enforced against the *resolved* IP, so a hostname resolving into a blocked range is rejected (#162 M3). See [native bridge](native-bridge.md). |
| `"tls certificate verification failed"` | The https server's certificate was rejected: untrusted chain (e.g. self-signed), expired, or its SANs don't cover the requested hostname/IP. Verification uses the system trust store; `SSL_CERT_FILE`/`SSL_CERT_DIR` are honoured for custom CAs. Distinguishable from `"http request failed"` so apps can tell a possible MITM from an unreachable host. |
...
**Shutdown behavior.** `http_client_destroy` waits up to 3 s for in-flight workers to drain naturally, then shuts down (`shutdown(2)`, not `close(2)`) the TCP sockets of any still parked inside `parse_response`'s blocking read, then waits up to 1 s more for them to unwind. Tearing down the read side surfaces as `Connection_Closed` inside `recv_tcp`, which lets the parser return through its error path with the scanner buffer freed (#156), while the fd itself stays owned by the worker so it is closed exactly once (#260 L1). The "N HTTP worker(s) still in flight at shutdown" warning only fires if a worker survives both phases, which should be rare in practice.
